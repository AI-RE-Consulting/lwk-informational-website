        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-10">
          ...
          <div>
            ...
            <a
              ...
              className="inline-block mt-5 text-white/40 hover:text-accent-gold transition-colors"
            >
          ...
          <nav className="flex flex-wrap gap-x-8 gap-y-3">
            {NAV_LINKS.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-white/50 text-xs font-medium tracking-[0.15em] uppercase hover:text-white transition-colors"
              >
                {link.label}
              </a>
            ))}
